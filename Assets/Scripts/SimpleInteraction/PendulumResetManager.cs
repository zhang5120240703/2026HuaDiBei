...
public class PendulumResetManager : MonoBehaviour
{
    ...
    /// 仅重置摆动角度（摆长保持不变）
    /// </summary>
    private void ResetAngleOnly()
    {
        if (dragControl == null) return;

        // 仅重置角度为0（最低点），摆长保持当前值
        dragControl.currentAngle = 0f;
        dragControl.UpdateBallPosition(); // 刷新摆球位置（基于当前摆长+0角度）

        // 同步摆长到其他脚本（仅同步当前值，不修改）
        if (pendulumCore != null)
        {
            pendulumCore.pendulumLength = dragControl.currentLength;
        }
        if (periodCounter != null)
        {
            periodCounter.pendulumLength = dragControl.currentLength;
        }
    }
