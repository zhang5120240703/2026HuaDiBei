...
2. 连接电路：按照项目预设的连线逻辑，完成电路搭建，此时滑块解锁，提示变为「电路已连接，请打开开关」（模型有限，仔细看节点在哪里）。
3. 运行实验：点击「电源开关」，提示变为「实验运行中」，屏幕上方显示波形（蓝色=电压波形、绿色=电流波形），灯光随电流亮度变化；拖动滑块可调整波形幅度。
4. 公式验证：在公式输入框输入「f=1/(2π√LC)」，点击「提交」，正确显示绿色提示，错误则显示正确公式。
5. 暂停/重置：再次点击「电源开关」可暂停实验（波形消失、灯光关闭）；点击「重置」按钮可恢复初始状态，重新进行实验。
